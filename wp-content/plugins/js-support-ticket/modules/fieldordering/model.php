...
//        $query = "SELECT * FROM `".jssupportticket::$_db->prefix."js_ticket_fieldsordering` WHERE published = 1 AND fieldfor = 1 ORDER BY ordering LIMIT ".JSSTpagination::getOffset().", ".JSSTpagination::getLimit();
    ...
    function storeUserField($data) {
        ...
        if ($data['isuserfield'] == 1) {
            ...
            if (isset($data['userfieldtype']) && $data['userfieldtype'] == 'depandant_field') {
                ...
                if (!empty($data['arraynames'])) {
                    ...
                    if($empty_flag == 0){
                        JSSTmessage::setMessage(__('Please Insert Atleat one value for every option', 'js-support-ticket'), 'error');
                        return 2 ;
                    }
                }
            }
        ...
        if(isset($data['cannotshowonlisting'])){
            $query_array['cannotshowonlisting'] = $data['cannotshowonlisting'];
        }
        if(isset($data['cannotsearch'])){
            $query_array['cannotsearch'] = $data['cannotsearch'];
        }
...
//echo '<pre>';print_r($query_array);die();
    ...
    function getFieldsForComboByFieldFor() {
        ...
        $query = "SELECT fieldtitle AS text ,id FROM " . jssupportticket::$_db->prefix . "js_ticket_fieldsordering WHERE fieldfor = $fieldfor AND (userfieldtype = 'radio' OR userfieldtype = 'combo' OR userfieldtype = 'depandant_field') ";
    ...
    function getOptionsForFieldEdit() {
        ...
        if ($data->isuserfield == 1) {
            $html .= '<a class="button" style="margin-left:10px;" id="user-field-anchor" href="?page=fieldordering&jstlay=adduserfeild&fieldfor='.$data->fieldfor.'&jssupportticketid=' . $data->id .'"> ' . __('Advanced', 'js-support-ticket') . ' </a>';
        }
    ...
    function deleteUserField($id){
        ...
        if ($this->userFieldCanDelete($result) == true) {
            ...
            JSSTmessage::setMessage(__('User Field','js-support-ticket').' '.__('in use cannot be deleted','js-support-ticket'), 'error');
        }
        return;
    }
